...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  // This way signup completes even if email fails
  try {
    new Email(newUser, url).sendWelcome().catch(err => {
      console.error('Welcome email failed:', err.message);
    });
  } catch (err) {
    console.error('Error initiating welcome email:', err.message);
  }
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordConfirm = req.body.passwordConfirm;
  await user.save();
  //  User.findByIdUpdate will not work  and will update password in unencrypted manner and other middle ware will have issue
  createSendToken(user, 200, res);
});
